...
int offsetX = 0, offsetY = 0, clickX=0, clickY=0, dragX=50, dragY=50, x = 0, y = 0, line_count=101, circle_count=101, circle_count_d=101, rect_count=0, rect_count_realtime=0, beat_count_realtime=0;
...
float[][] shells_property_array = {{-5000, 10, 100}, {-5000, 0.05, 100}, {-5000, 0.01, 100}, {-5000, 0.1, 100}};
...
void draw() {
  ...
  faces = opencv.detect();
  if (faces.length != 0) {
    camera(10*faces[0].x, 10*faces[0].y, (height/2.0) / tan(PI*30.0 / 180.0), 50.0, 50.0, 0.0, 0.0, 1.0, 0.0);
  }

  volumeIn = map(in.left.level(), 0, 0.5, 0, 2);
  ...
  //spotLight(255, 0, 0, mouseX, mouseY, 0, width/2, height/2, 100, 30, 0.2);
  ambient(0, 255, 0);
  emissive(255, 0, 0);
  shininess(255);
    ...
    for (int shell_count=0; shell_count<shells_property_array[shells_id][2]; ++shell_count) {
      ...
      for (int iv = 0; iv <= ivMax; iv++) {
        for (int iu = 0; iu <= iuMax; iu++) {
          float r1 = (bpm + 10*volumeIn + 0.1 * key_vibration) * random(-1, 1);
          float r2 = (bpm + 10*volumeIn + 0.1 * key_vibration) * random(-1, 1);
          float r3 = (bpm + 10*volumeIn + 0.1 * key_vibration) * random(-1, 1);
          colorMode(HSB, 2);
